...
[MCP](https://modelcontextprotocol.io) server exposing the Sotaproject SDK as
two agent tools — `sotaproject_list` and `sotaproject_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/sotaproject-mcp)
make build

# 2. Provide credentials via the environment
export SOTAPROJECT_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user sotaproject \
  -- /absolute/path/to/sotaproject-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./sotaproject-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// sotaproject_list: first page of records
{ "entity": "publication" }
{ "entity": "publication", "query": { } }

// sotaproject_load: one record by id
{ "entity": "publication", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/sotaproject-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user sotaproject \
     -- "$PWD"/dist/*/sotaproject-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `sotaproject_list` and `sotaproject_load` tools now appear
   in new sessions. Ask the agent to *"list publication using sotaproject"*
   and it calls `sotaproject_list` with `{"entity":"publication"}`.
...
```sh
export SOTAPROJECT_APIKEY=sk_live_xxx            # API key
export SOTAPROJECT_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `sotaproject_list` tool
...
### Call the `sotaproject_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `sotaproject_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `sotaproject_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `SOTAPROJECT_APIKEY` | API key sent with every request. |
| `SOTAPROJECT_BASE` | Optional override of the API base URL. |
...
```sh
./sotaproject-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"sotaproject_load","arguments":{"entity":"publication","query":{"id":1}}}}'
```
