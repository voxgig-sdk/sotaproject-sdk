...
class PublicationEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    base = SotaprojectSDK.test(seed, nil)
    seen = base.Publication(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Publication(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set SOTAPROJECT_TEST_PUBLICATION_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def publication_basic_setup(extra)
  ...
  entid_env_raw = ENV["SOTAPROJECT_TEST_PUBLICATION_ENTID"]
  ...
  if env["SOTAPROJECT_TEST_LIVE"] == "TRUE"
    ...
    client = SotaprojectSDK.new(Helpers.to_map(merged_opts))
  end
