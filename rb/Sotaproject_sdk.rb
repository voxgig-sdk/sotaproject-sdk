...
require_relative 'Sotaproject_types'


class SotaprojectSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = SotaprojectHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = SotaprojectHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, SotaprojectFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    SotaprojectUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = SotaprojectHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = SotaprojectHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = SotaprojectHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => SotaprojectError.new(
        "#{op}_allow",
        "SotaprojectSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = SotaprojectHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = SotaprojectError.new(
        "graphql_error", "SotaprojectSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = SotaprojectSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
