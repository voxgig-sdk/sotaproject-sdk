...
class PublicationEntity
  ...
  def make
    opts = @_entopts.dup
    PublicationEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = SotaprojectHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = SotaprojectHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  #
  # @param reqmatch [PublicationListMatch, Hash, nil] match filter (any subset of
  #   Publication fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Publication>, Array] the matching Publication items; raises SotaprojectError on failure
