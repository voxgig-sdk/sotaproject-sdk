...
boru-driven command-line client **and** interactive REPL for the Sotaproject
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/sotaproject-cli)
make build

# 2. See usage (words, entities, env vars)
./sotaproject-cli --help

# 3. Provide credentials once, via the environment
export SOTAPROJECT_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./sotaproject-cli list publication

# 5. Override the API base URL for a single call
SOTAPROJECT_BASE=https://api.example.com ./sotaproject-cli list publication

# 6. No arguments -> interactive REPL
./sotaproject-cli
sotaproject> list publication
sotaproject> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/sotaproject-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/sotaproject-cli list publication
   ```
...
Run with no arguments to open a REPL (prompt `sotaproject>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `publication`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/sotaproject-cli`. |
...
(`./sotaproject-cli <expr>`) and interactively (the REPL), and expressions compose the
