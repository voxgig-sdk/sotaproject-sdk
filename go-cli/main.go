// Command Sotaproject-cli is a CLI + REPL for the Sotaproject SDK,
...
const prompt = "sotaproject"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "publication"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      SOTAPROJECT_APIKEY (api key), SOTAPROJECT_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: SOTAPROJECT_APIKEY carries the API key and
	// SOTAPROJECT_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("SOTAPROJECT_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("SOTAPROJECT_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewSotaprojectSDK(opts)
