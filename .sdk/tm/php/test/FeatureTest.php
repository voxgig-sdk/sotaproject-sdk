...
class FtCtrl extends SotaprojectControl
{
...
class FtClient
{
    ...
    private SotaprojectUtility $utility;

    public function __construct(SotaprojectUtility $utility, array $options)
    {
    ...
    public function get_utility(): SotaprojectUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (SotaprojectContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public SotaprojectContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (SotaprojectContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = SotaprojectConfig::make_feature($name);
    ...
    private static function buildUrl(SotaprojectSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof SotaprojectError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new SotaprojectResponse($fetched);
        }
    ...
    private function fail(SotaprojectContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (SotaprojectContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
