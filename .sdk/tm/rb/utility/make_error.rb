...
module SotaprojectUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = SotaprojectContext.new({}, nil)
    end
    op = ctx.op || SotaprojectOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || SotaprojectResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(SotaprojectError) ? err.msg : err.to_s
    ...
    sdk_err = SotaprojectError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(SotaprojectError)
