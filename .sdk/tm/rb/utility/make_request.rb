...
module SotaprojectUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = SotaprojectResponse.new({})
    ...
    if fetch_err
      ...
      response = SotaprojectResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = SotaprojectResponse.new(fetched)
    else
