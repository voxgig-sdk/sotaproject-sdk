...
local PublicationEntity = {}
PublicationEntity.__index = PublicationEntity


function PublicationEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PublicationEntity)
...
function PublicationEntity:get_name()
  return self._name
end


function PublicationEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PublicationEntity.new(self._client, opts)
end
...
function PublicationEntity:mark_deleted()
  self._deleted = true
end


function PublicationEntity:deleted()
  return true == self._deleted
end


function PublicationEntity:data_set(args)
  if args ~= nil then
...
function PublicationEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PublicationEntity:match_set(args)
  if args ~= nil then
...
function PublicationEntity:match_get()
...
function PublicationEntity:stream(action, args, callopts)
...
function PublicationEntity:_run_op(ctx, post_done)
  local utility = self._utility
