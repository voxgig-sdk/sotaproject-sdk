...
// Sotaproject SDK Publication entity
...
class PublicationEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new PublicationEntity($this->_client, $opts);
    }

    /**
     * @param Publication|array $args Publication data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = SotaprojectHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Publication|array The current Publication data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Publication fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = SotaprojectHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Publication fields).
     */
     ...
     *
     * @param PublicationListMatch|array|null $reqmatch Match filter (any subset
     *   of Publication fields) as an assoc-array; PublicationListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Publication[]|array A list of Publication items as assoc-arrays at
     *   the SDK boundary; throws SotaprojectError on failure (item-5 convention).
     */
