...
from sotaproject_sdk.sotaproject_types import (
    Publication,
...
class PublicationEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return PublicationEntity(self._client, opts)
    ...
    def match_get(self) -> Publication:
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Publication]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Publication().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
