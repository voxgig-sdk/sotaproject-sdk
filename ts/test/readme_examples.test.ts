...
const TEST_SEED = {"entity":{"publication":{"test01":{"id":"test01"}}}}
...
function isRunnable(code: string): boolean {
  return (
    /new\s+SotaprojectSDK\b/.test(code) ||
    /\bSotaprojectSDK\.test\b/.test(code) ||
    /\bclient\s*\./.test(code)
  )
}
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+SotaprojectSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/SotaprojectSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
