...
import (
	...
	omni "github.com/voxgig-sdk/sotaproject-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.SotaprojectError); is {
				out := map[string]any{
					"name":    "SotaprojectError",
...
func omniCtx(arg any, client *sdk.SotaprojectSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.SotaprojectSDK); is {
			client = live
